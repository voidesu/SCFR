...
void gerarParcelas(Vendas v){
    ...
    Data venc = v.dataVenda;
    float valorBase = v.valorTotalVenda / limite;
    ...
    for(i = 1; i <= limite; i++){
        ...
        p.idParcela = 1001 + qtdParcelas;
        p.idVenda = v.idVenda;
        p.numeroDaParcela = i;

        // POW = POTENCIA DE UM NUMERO
        p.valorDaParcela = valorBase * pow(1.01, i);

        // VENCE SOMENTE DAQUI 30 DIAS
        venc.dia += 30;
